<script>
	import { exportedJson, data, handleSpreadsheetFile, spreadsheetConversion } from '../src/lib/Conversion'
...
{#each [['importList', 'importInput', 'import'], ['exportList', 'exportInput', 'export']] as list, index}
	<form on:submit|preventDefault={() => (data[list[1]] ? addToList(list[0], list[1]) : '')}>
		<label for={`input${index}`}>Columns to {list[2]}</label>
		<input id={`input${index}`} bind:value={data[list[1]]} type="text" placeholder="Column name" />
	...
	{#each ['importList', 'exportList'] as list}
		<div class="list">
			{#each data[list] as item, index}
				...
				<button class="button" on:click={() => removeFromList(list, index)}>❌</button>
			{/each}
		</div>
	{/each}
</div>
